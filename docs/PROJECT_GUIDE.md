...
- **Derived regions and boundaries:** most utilities don't say which town or district an outage is in, but all of them give coordinates. The dashboard now looks up the nearest Arizona town (from the US Census place list, fully offline) and shows it as "**≈ Marana**" — the ≈ marks it as an estimate, distinct from utility-published names.
